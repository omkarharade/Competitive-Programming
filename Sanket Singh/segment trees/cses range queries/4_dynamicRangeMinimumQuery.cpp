...
void update(vll &arr, vll &segTree, int s, int e, int tIdx, int i, ll val) {
...
ll query(vll &segTree, int s, int e, int tIdx, int l, int r) {

	if (l > e or r < s) return 1e18;
	if (s >= l and e <= r) return segTree[tIdx];

	int mid = s + (e - s) / 2;

	ll leftMin = query(segTree, s, mid, 2 * tIdx, l, r);
	ll rightMin = query(segTree, mid + 1, e, 2 * tIdx + 1, l, r);
...
void solve() {
	...
	while (q--) {
		...
		if (type == 1) {
			...
			update(arr, segTree, 0, n - 1, 1, i, val);
		}
		else {
			int l, r;
			cin >> l >> r;
