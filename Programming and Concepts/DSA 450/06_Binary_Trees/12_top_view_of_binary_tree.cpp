...
vector<int> topView(Node *root) {
	...
	while (!q.empty()) {
		...
		for (int i = 0; i < size; i++) {
			...
			// checking if there is a element with the currHIndex key
			// if not, then add the node's data into map

			// as this means it is the first element for that
			// particular horizontal index

			if (traversalMap.find(currHIndex) == traversalMap.end()) {
